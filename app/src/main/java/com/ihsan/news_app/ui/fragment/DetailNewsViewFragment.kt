...
class DetailNewsViewFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (!TextUtils.isEmpty(article.urlToImage)) {
            Picasso.get().load(article.urlToImage).fit().placeholder(R.drawable.progress_animation)
        ...
        binding.btnCardViewOnline.setOnClickListener {
            val action = article.let { it1 ->
                DetailNewsViewFragmentDirections.actionDetailNewsViewFragmentToWebviewNewsFragment(
                    it1.url.toString()
                )
            }
        ...
        binding.detailBookmarks.setOnClickListener {
            ...
            if (!article.isBookmarked) {
                ...
                Snackbar.make(it, "Bookmarks Removed from ${article.title}", Snackbar.LENGTH_SHORT)
                    .show()
                binding.detailBookmarks.setImageResource(R.drawable.ic_bookmark_add)
